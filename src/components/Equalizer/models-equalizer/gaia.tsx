...
const EqualizerGaia = ({
  ...
  const handleParameterChange = async (
    bandId: number,
    parameter: 'frequency' | 'gain' | 'quality',
    value: number,
  ) => {
    setBands(prevBands =>
      prevBands.map(band =>
        band.id === bandId ? {...band, [parameter]: value} : band,
      ),
    );

    const success = await sendEQParameter(bandId, parameter, value);

    if (success) {
      const log = `SET_EQ_PARAMETER band=${bandId} ${parameter}=${value}`;
      setLogs(l => [log, ...l].slice(0, 5));

      // Se for frequency, reenvia gain e quality
      if (parameter === 'frequency') {
        const band = bands.find(b => b.id === bandId)!;
        setTimeout(async () => {
          await sendEQParameter(bandId, 'gain', band.gain);
          await sendEQParameter(bandId, 'quality', band.quality);
        }, 50);
      }
    }
  };
    ...
    <ScrollView
      ...
      <View style={styles.controlsContainer}>
        ...
        <View style={styles.parameterContainer}>
          ...
          <Text style={styles.parameterValue}>
            {currentBand.frequency.toFixed(0)} Hz
          </Text>
          <Slider
            style={styles.slider}
            minimumValue={20}
            ...
            onValueChange={value =>
              handleParameterChange(activeBand, 'frequency', value)
            }
        ...
        <View style={styles.parameterContainer}>
          ...
          <Slider
            ...
            onValueChange={value =>
              handleParameterChange(activeBand, 'gain', value)
            }
            minimumTrackTintColor="#000"
            maximumTrackTintColor="#E0E0E0"
          />
        ...
        <View style={styles.parameterContainer}>
          ...
          <Slider
            ...
            onValueChange={value =>
              handleParameterChange(activeBand, 'quality', value)
            }
            minimumTrackTintColor="#000"
            maximumTrackTintColor="#E0E0E0"
          />
      ...
      <View style={styles.filterInfoContainer}>
        <Text style={styles.filterInfoText}>
          Filter Type: PEQ (Parametric Equalizer)
        </Text>
      </View>
        ...
        {bands.map(band => (
          <View key={band.id} style={styles.bandStatus}>
            <Text style={styles.bandStatusText}>
              Banda {band.id}: {band.frequency}Hz, {band.gain.toFixed(1)}dB, Q
              {band.quality.toFixed(2)}
            </Text>
          </View>
        ))}
      </View>
      ...
      <View style={styles.statusContainer}>
        ...
        <Text style={styles.bandStatusText}>{logs.join('\n')}</Text>
      </View>
